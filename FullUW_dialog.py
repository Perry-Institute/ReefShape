...
class FullWorkflowDlg(QtWidgets.QDialog):
    ...
    def __init__(self, parent):
        ...
        self.setMinimumSize(585, 0.6*parent.frameGeometry().height())
        self.setFixedWidth(585)
        x = parent.frameGeometry().width()/2.0 - 292 # set starting position so that widget is roughly centered on screen
        if(x<0): x = 0
        y = parent.frameGeometry().height()/2.0 - 0.4*parent.frameGeometry().height()
        self.setGeometry(parent.frameGeometry().x() + x, parent.frameGeometry().y() + y, 585, 0.85*parent.frameGeometry().height())
    ...
    def runWorkFlow(self):
        ...
        # if( no existing dem file):
        self.chunk.exportRaster(path = self.output_dir + "/" + self.project_name + "_" + self.chunk.label + "_DEM.tif", resolution = DEM_RES, nodata_value = -5,
