...
<b>`11_gridded_rugosity.py`</b> This script computes the 3D/2D surface area ratio (rugosity) on a 1-meter grid within the currently selected chunk's outer boundary polygon, and imports the result back into the chunk as a labeled DEM ("Rugosity") alongside (not replacing) the project's real DEM. Like `09_calculate_area_ratio.py`, it uses the full 3D mesh rather than a 2.5D DEM, so overhangs are captured correctly. Unlike that script, it doesn't duplicate-and-clip the mesh per cell — instead it iterates the mesh's triangles exactly once, computes each triangle's 3D area, and accumulates it into the 1-meter cell containing the triangle's centroid. Per-cell rugosity is the accumulated 3D area divided by the cell's 1 m² footprint, so an overhang (two slabs of mesh stacked over one cell) gives rugosity ≈ 2. The grid is derived from the boundary polygon's bounding box snapped to the cell size, so two timepoints with the same boundary produce pixel-aligned rasters that can be subtracted, regressed, or differenced directly. The dialog also has a "Also save raster to disk" checkbox — when enabled, the GeoTIFF is also copied to a user-selected folder as `<project>_<chunk>_rugosity_1m.tif`; otherwise it's only kept inside the chunk. Requires a 3D model and an OuterBoundary polygon in the chunk; depends on `rasterio` (auto-installed on first run via the `pip_auto_install` helper).
