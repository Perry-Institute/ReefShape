...
<b>`08_create_boundary_from_photos.py`</b> This script generates an outer boundary polygon by tracing the outline of the aligned camera positions, rather than from corner markers. It's especially useful after using `03_align_chunks_ICP.py` (where you may have no shared markers to define a boundary), or any time you want the ROI to follow your actual photo coverage rather than a marker-to-marker rectangle. The user can specify the photo footprint radius (auto-estimated from camera altitude and FOV but user-adjustable) and a number of smoothing passes (Chaikin corner-cutting; defaults to 2, gives a visibly smooth polygon). Holes inside the coverage area are filled, and only the largest connected region is kept so a few stray cameras don't pull the boundary out of shape. The resulting polygon is added to the chunk as the OuterBoundary, ready to be used by the area ratio script or to clip exported data products.
